...
$allClosed = "select sum(aa.timeTaken) as hrs1 from (SELECT distinct o.* FROM openssbtime o, closedssb c WHERE o.ssbID = c.ssbID and o.owner='$dat1[adminID]' and c.owner='$dat1[adminID]' and MONTH(c.closedOn)='$mon' and YEAR(c.closedOn)='$year') aa";
...
<?php
 ...
 <td>
 <?php
$allClosedLead = "select sum(aa.timeTaken) as hrs1 from (SELECT distinct o.* FROM openssbtime o, closedssb c WHERE o.ssbID = c.ssbID and o.owner='$lead' and c.owner='$lead' and MONTH(c.closedOn)='$mon' and YEAR(c.closedOn)='$year') aa";
...
<?php 
$allOwner11="select name,adminID,photograph from webadmin where userRole = 3 and teamLead='$lead'";
$ownerData11=mysql_query($allOwner11);
while($dat11=mysql_fetch_array($ownerData11)){
	$allClosed1 = "select sum(aa.timeTaken) as hrs1 from (SELECT distinct o.* FROM openssbtime o, closedssb c WHERE o.ssbID = c.ssbID and o.owner='$dat11[adminID]' and c.owner='$dat11[adminID]' and MONTH(c.closedOn)='$mon' and YEAR(c.closedOn)='$year') aa";

	$close1=mysql_query($allClosed1) or die(mysql_error());
	$closedArr1=mysql_fetch_array($close1);
	@$closedSum1 += $closedArr1['hrs1']; 
 }

$allOwner11="select name,adminID,photograph from webadmin where userRole = 3 and teamLead='$lead'";
$ownerData11=mysql_query($allOwner11);
while($dat11=mysql_fetch_array($ownerData11)){
	$billedTime1="SELECT sum(hours2) as billed FROM   closedssb WHERE  owner='$dat11[adminID]' && MONTH(closedOn)='$mon' && YEAR(closedOn)='$year'";
		 
	$billed1=mysql_query($billedTime1) or die(mysql_error());
	$billedArr1=mysql_fetch_array($billed1);
	@$billedSum1 += $billedArr1['billed']; 
}
 ?>
 ...
 <ul class="time-sheet-hrs">
	...
	<li class="" style="color:#3399FF">TimeSheet Hours: <?php echo $closedSum1+$closedLead; ?> <sup>Hrs.</sup></li>

	<li class="" style="color:#ED9B4D">Billed Hours:  <?php echo $billedSum1+$billedLeadTime; ?> <sup>Hrs.</sup></li>
