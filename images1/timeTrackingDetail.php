...
<h2> </h2>
		<div id="efficiency-wrapper">
			...
			<ul>
					<?php $allOwner12="select name,adminID from webadmin where userRole = 3 and status = 1 and teamLead='$lead'";
					...
					$allClosed = "select sum(aa.timeTaken) as hrs1 from (SELECT distinct o.* FROM openssbtime o, closedssb c WHERE o.ssbID = c.ssbID and o.owner='$dat12[adminID]' and c.owner='$dat12[adminID]' and MONTH(c.closedOn)='$mon' and YEAR(c.closedOn)='$year' and c.isManual !='1') aa";
...
<span class = 'li-efficiency' >Efficiency :</span> <span class = 'li-billed-hour'><?php   echo $efficiency." %"; ?></span>
					<!-------------------------Efficiency End ------------------->
						...
						<?php
  $allOwner1="select name,adminID from webadmin where userRole = 3 and status = 1 and teamLead='$lead'";
