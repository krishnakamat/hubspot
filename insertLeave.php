...
if(!empty($_POST['date2']) && $type==2){
	...
	//$diff=0;
	echo $diff = $diff + (strtotime($date2)- strtotime($date2))/24/3600 + 1;
...
if($halfRowTwice > 0){
		header("location:leaveTracking.php?msg=Twice");
}	
elseif($fullRowTwice > 0){
		header("location:leaveTracking.php?msg=Twice");
}	
elseif($diff <= $accuratebal && $diff!=0 && $type==1){
	$query1 = "INSERT INTO `empleavedetails` (`leaveID`, `empID`, `appliedOn`, `typeLeave`, `appliedFor`, `Notes`) VALUES('', '$adminid', '$appliedOn', '$type', '$date', '$notes')";
	...
	$subject = "Leave Application from $empName";

	$msg="<p>Hello Administrator</p>
			<br/>
			<p>Employee <b>$empName</b> &nbsp; has applied for Half Day leave on the date(s) below </p>
			<p>Date: <b>$date</b></p>
			<p>Notes: $notes</p>
  ...
  //send email
	...
	if(!empty($appliedStatus)){
		header("location:leaveTracking.php?msg=success&status=$appliedStatus");
	}else{
		header("location:leaveTracking.php?msg=success");
	}
...
elseif($diff <= $accuratebal && $type==2 && $diff!=0){
	...
	$query2 = "INSERT INTO `empleavedetails` (`leaveID`, `empID`, `appliedOn`, `typeLeave`, `appliedFor`, `Notes`) VALUES('', '$adminid', '$appliedOn', '$type', '$date2', '$notes')";
	...
	$msg="<p>Hello Administrator</p>
			<br/>
			<p>Employee <b>$empName</b> &nbsp; has applied for Full Day leave on the date(s) below </p>
...
elseif($totalholiday==1){
	header("location:leaveTracking.php?holiday=true");
}
elseif($diff==0){
		header("location:leaveTracking.php?weekend=true");
}
else{
header("location:leaveTracking.php?msg=Err");
}
